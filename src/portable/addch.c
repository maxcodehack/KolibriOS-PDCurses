...
{
#ifdef PDCDEBUG
	if (trace_on) PDC_debug("addch() - called\n");
#endif

	return( PDC_chadd( stdscr, ch, (bool)(!(_cursvar.raw_out)), TRUE ) );
}
/***********************************************************************/
int	waddch(WINDOW *win, chtype ch)
/***********************************************************************/
{
	...
	return( PDC_chadd( win, ch, (bool)(!(_cursvar.raw_out)), TRUE ) );
}
/***********************************************************************/
int	mvaddch(int y, int x, chtype ch)
/***********************************************************************/
{
#ifdef PDCDEBUG
	if (trace_on) PDC_debug("mvaddch() - called\n");
#endif
	if (move(y,x) == ERR)
		return(ERR);
	return( PDC_chadd( stdscr, ch, (bool)(!(_cursvar.raw_out)), TRUE ) );
...
{
#ifdef PDCDEBUG
	if (trace_on) PDC_debug("mvwaddch() - called\n");
#endif
...
{
#ifdef PDCDEBUG
	if (trace_on) PDC_debug("echochar() - called\n");
#endif

	if (PDC_chadd( stdscr, ch, (bool)(!(_cursvar.raw_out)), TRUE ) == ERR)
		return(ERR);
	return(refresh());
}
/***********************************************************************/
...
{
#ifdef PDCDEBUG
	if (trace_on) PDC_debug("wechochar() - called\n");
#endif

	if (PDC_chadd( win, ch, (bool)(!(_cursvar.raw_out)), TRUE ) == ERR)
		return(ERR);
	return(wrefresh(win));
}
